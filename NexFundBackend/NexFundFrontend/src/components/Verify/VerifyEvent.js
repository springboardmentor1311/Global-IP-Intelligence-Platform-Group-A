...
import { Calendar, ArrowLeft, IndianRupee, Mail, Check, X, Clock } from 'lucide-react';
...
const VerifyEvent = () => {
  ...
  // Get user's events
  const userEvents = user ? getUserFundraisers(user.username).filter(f => f.type === 'event') : [];

  // Get all payments for user's events
  const eventPayments = payments.filter(payment => 
    userEvents.some(event => event.id === payment.fundraiserId)
  );

  const handleVerifyPayment = (paymentId) => {
    setVerificationStatus(prev => ({ ...prev, [paymentId]: 'verified' }));
    toast.success('Payment verified! Confirmation email will be sent to participant.');
  };
      ...
      <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <div className="flex items-center space-x-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900">Event Participants Verification</h1>
          </div>
          <p className="text-gray-600">Verify payments and send event confirmation emails to participants</p>
        </div>

        {eventPayments.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
            <Calendar className="h-12 w-12 text-gray-300 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-500 mb-2">No Event Participants Yet</h3>
            <p className="text-gray-400">When people register for your events, they will appear here for verification.</p>
          </div>
        ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {eventPayments.map((payment) => {
                    const event = userEvents.find(e => e.id === payment.fundraiserId);
