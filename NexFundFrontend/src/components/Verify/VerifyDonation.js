...
import { Heart, ArrowLeft, IndianRupee, Check, X, Clock } from 'lucide-react';
...
const VerifyDonation = () => {
  ...
  const userDonations = user ? getUserFundraisers(user.username).filter(f => f.type === 'donation') : [];

  // Get all payments for user's donations
  const donationPayments = payments.filter(payment => 
    userDonations.some(donation => donation.id === payment.fundraiserId)
  );

  const handleVerifyPayment = (paymentId) => {
    setVerificationStatus(prev => ({ ...prev, [paymentId]: 'verified' }));
    toast.success('Donation verified! Thank you message sent to contributor.');
  };
      ...
      <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <div className="flex items-center space-x-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900">Donation Contributors Verification</h1>
          </div>
          <p className="text-gray-600">Verify donations and acknowledge contributors</p>
        </div>

        {donationPayments.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
            <Heart className="h-12 w-12 text-gray-300 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-500 mb-2">No Donation Contributors Yet</h3>
            <p className="text-gray-400">When people donate to your causes, they will appear here for verification.</p>
          </div>
        ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {donationPayments.map((payment) => {
                    const donation = userDonations.find(d => d.id === payment.fundraiserId);
                    ...
                    return (
                      <tr key={payment.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{donation?.title}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
